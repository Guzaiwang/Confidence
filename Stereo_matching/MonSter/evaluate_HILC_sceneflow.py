...
from monster_HILC import Monster, autocast
...
def get_coff(pred, confi):
    valid_indices = np.isfinite(pred) & np.isfinite(confi)
    pred = pred[valid_indices]
    confi = confi[valid_indices]
    pearson_corr, pearson_p = pearsonr(pred, confi)
    spearman_corr, spearman_p = spearmanr(pred, confi)
...
def validate_sceneflow(model, iters=32, mixed_prec=False):
    ...
    for val_id in tqdm(range(len(val_dataset))):
        ...
        with autocast(enabled=mixed_prec):
            start = time.time()
            flow_pr, la, alpha, beta = model(image1, image2, iters=iters, test_mode=True)
            end = time.time()
        ...
        epistemic = padder.unpad(epistemic).cpu().squeeze(0)
        ...
        try:
            ep_pearson, ep_spearman = get_coff(epistemic.numpy().flatten()[val.numpy()], epe.numpy()[val.numpy()])
        except:
            ep_spearman = 0
        ...
        epistemic_value = epistemic.numpy().flatten()[val.numpy()].mean()
        if np.isnan(epistemic_value):
            epistemic_value = 0
        else:
            epistemic_all += epistemic_value
            epistemic_idx += 1
        if val_id < 9 or (val_id + 1) % 10 == 0:
            logging.info(
        ...
        out_list.append(out[val].cpu().numpy())
        epistemic_list.append(ep_spearman)
        print("epistemic mean value is {}, spearman coffi is {}".format(epistemic_value, ep_spearman))
    ...
    epe = np.mean(epe_list)
    d1 = 100 * np.mean(out_list)
    epistemic_spearman = np.mean(epistemic_list)
    epistemic_avg = epistemic_all / epistemic_idx
    ...
    print("epistemic mean value is",epistemic_avg)
    print("epistemic spearman is", epistemic_spearman)
